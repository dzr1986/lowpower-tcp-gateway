...
std::optional<Frame> parseFrame(const std::vector<uint8_t>& data) {
    ...
    if (data.size() != HEADER_SIZE + f.size) {
        return std::nullopt;
    }

    if (f.size > 0) {
        f.payload.assign(data.begin() + HEADER_SIZE, data.end());
    }

    return f;
}

Frame makeFrame(MsgType type, uint8_t flag, const std::vector<uint8_t>& payload) {
    Frame f;
    f.version = VERSION_1;
    f.type = type;
    f.flag = flag;
    f.size = static_cast<uint16_t>(payload.size());
    f.payload = payload;
    return f;
}
...
std::vector<uint8_t> makeHeartbeatFrame() {
    Frame f = makeFrame(MsgType::HEARTBEAT, 0, {});
...
std::vector<uint8_t> makeWakeupFrame(const std::string& local_key) {
    ...
    writeU32BE(payload, c);

    Frame f = makeFrame(MsgType::WAKEUP, 0, payload);
    return encodeFrame(f);
}
...
std::optional<AuthRequest> parseAuthRequestJson(const std::vector<uint8_t>& plain_json) {
    try {
...
AuthResponseResult buildAuthResponse(
    const std::string& devid,
    const std::string& local_key,
    ...
    RAND_bytes(rnd.data(), static_cast<int>(rnd.size()));
    ...
    std::vector<uint8_t> resp_plain(resp_str.begin(), resp_str.end());

    auto encrypted_data = aes128CbcPkcs7Encrypt(local_key, iv, resp_plain);
    ...
    auto lv = buildAuthPayloadLV(iv, encrypted_devid_b64, encrypted_data);

    Frame frame = makeFrame(
        MsgType::AUTH_RESP,
        static_cast<uint8_t>(EncryptFlag::AES_CBC_128_PKCS7),
        lv
    );

    AuthResponseResult result;
    result.frame = frame;
    result.server_random = server_random;
